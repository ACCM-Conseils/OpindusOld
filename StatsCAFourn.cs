...
{
	/// <summary>
	/// Description résumée de StatsCAFourn.
	/// </summary>
	public class StatsCAFourn : System.Windows.Forms.Form
	{
		...
		public StatsCAFourn(int Agence)
		{
			//
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(StatsCAFourn));
			...
			this.textBox1.Text = "Statistiques du chiffre d\'affaires fournisseurs sur les 5 dernières années";
			...
			this.mtgcComboBox1.Size = new System.Drawing.Size(352, 21);
			...
			this.radioButton2.Text = "Impression d\'un fournisseur";
			...
			this.radioButton1.Text = "Impression états tous fournisseurs";
			// 
			// oleDbDataAdapter1
			// 
			...
			this.printPreviewDialog1.Location = new System.Drawing.Point(557, 17);
			...
			this.Name = "StatsCAFourn";
			this.ShowInTaskbar = false;
			this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
			this.Text = "Statistiques C.A. Fournisseurs";
		...
		private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
		{
			...
			e.Graphics.DrawString("Statistiques du chiffre d'affaires fournisseurs", new Font("Arial", 20, FontStyle.Bold), Brushes.Black, e.MarginBounds.Left+20, e.MarginBounds.Top);
			ypos+=40;
			if(Agence.Equals(1))
		...
		//	ypos+=27;
			...
			while(ypos<e.MarginBounds.Height&&(countRow<ds.Tables["fournisseurs"].Rows.Count)&&(ds.Tables["fournisseurs"].Rows.Count>0))
			{
				if(ancienfourn!=ds.Tables["fournisseurs"].Rows[countRow]["nom"].ToString())
				{
					Comm.CommandText="SELECT Sum(t_devis_lignes.Quantite*t_devis_lignes.[prix achat net]) AS Expr1 FROM T_fournisseurs INNER JOIN (T_devis_entete INNER JOIN (t_devis_lignes INNER JOIN T_pieces_detachees ON t_devis_lignes.[Code piece] = T_pieces_detachees.Identificateur) ON T_devis_entete.[N° Devis] = t_devis_lignes.[N° Devis]) ON T_fournisseurs.Code = T_pieces_detachees.[Code Fournisseur Principal] WHERE (((T_devis_entete.[Date reparation])>=#1/1/"+annee_encours+"# And (T_devis_entete.[Date reparation])<=#12/31/"+annee_encours+"#)) GROUP BY T_fournisseurs.Nom HAVING (((T_fournisseurs.Nom)='"+ds.Tables["fournisseurs"].Rows[countRow]["nom"].ToString()+"'))"; 
					...
					Comm.CommandText="SELECT Sum(T_devis_lignes.[Prix achat net]*T_devis_lignes.Quantite) AS Expr1 FROM T_fournisseurs INNER JOIN (T_devis_entete INNER JOIN (T_pieces_detachees INNER JOIN T_devis_lignes ON T_pieces_detachees.Identificateur = T_devis_lignes.[Code piece]) ON T_devis_entete.[N° Devis] = T_devis_lignes.[N° Devis]) ON T_fournisseurs.Code = T_pieces_detachees.[Code Fournisseur Principal] WHERE (((T_devis_entete.[Date reparation])>=#1/1/"+annee_1+"# And (T_devis_entete.[Date reparation])<=#12/31/"+annee_1+"#)) GROUP BY T_fournisseurs.Nom HAVING (((T_fournisseurs.Nom)='"+ds.Tables["fournisseurs"].Rows[countRow]["nom"].ToString()+"'))"; 
					DataAD.SelectCommand=Comm;
					DataAD.Fill(ds,"n1");
					try
					{
					...
					Comm.CommandText="SELECT Sum(T_devis_lignes.[Prix achat net]*T_devis_lignes.Quantite) AS Expr1 FROM T_fournisseurs INNER JOIN (T_devis_entete INNER JOIN (T_pieces_detachees INNER JOIN T_devis_lignes ON T_pieces_detachees.Identificateur = T_devis_lignes.[Code piece]) ON T_devis_entete.[N° Devis] = T_devis_lignes.[N° Devis]) ON T_fournisseurs.Code = T_pieces_detachees.[Code Fournisseur Principal] WHERE (((T_devis_entete.[Date reparation])>=#1/1/"+annee_2+"# And (T_devis_entete.[Date reparation])<=#12/31/"+annee_2+"#)) GROUP BY T_fournisseurs.Nom  HAVING (((T_fournisseurs.Nom)='"+ds.Tables["fournisseurs"].Rows[countRow]["nom"].ToString()+"'))";  
					DataAD.SelectCommand=Comm;
					DataAD.Fill(ds,"n2");
					try
					{
					...
					Comm.CommandText="SELECT Sum(T_devis_lignes.[Prix achat net]*T_devis_lignes.Quantite) AS Expr1 FROM T_fournisseurs INNER JOIN (T_devis_entete INNER JOIN (T_pieces_detachees INNER JOIN T_devis_lignes ON T_pieces_detachees.Identificateur = T_devis_lignes.[Code piece]) ON T_devis_entete.[N° Devis] = T_devis_lignes.[N° Devis]) ON T_fournisseurs.Code = T_pieces_detachees.[Code Fournisseur Principal] WHERE (((T_devis_entete.[Date reparation])>=#1/1/"+annee_3+"# And (T_devis_entete.[Date reparation])<=#12/31/"+annee_3+"#)) GROUP BY T_fournisseurs.Nom  HAVING (((T_fournisseurs.Nom)='"+ds.Tables["fournisseurs"].Rows[countRow]["nom"].ToString()+"'))"; 
					DataAD.SelectCommand=Comm;
					DataAD.Fill(ds,"n3");
					try
					{
					...
					Comm.CommandText="SELECT Sum(T_devis_lignes.[Prix achat net]*T_devis_lignes.Quantite) AS Expr1 FROM T_fournisseurs INNER JOIN (T_devis_entete INNER JOIN (T_pieces_detachees INNER JOIN T_devis_lignes ON T_pieces_detachees.Identificateur = T_devis_lignes.[Code piece]) ON T_devis_entete.[N° Devis] = T_devis_lignes.[N° Devis]) ON T_fournisseurs.Code = T_pieces_detachees.[Code Fournisseur Principal] WHERE (((T_devis_entete.[Date reparation])>=#1/1/"+annee_4+"# And (T_devis_entete.[Date reparation])<=#12/31/"+annee_4+"#)) GROUP BY T_fournisseurs.Nom  HAVING (((T_fournisseurs.Nom)='"+ds.Tables["fournisseurs"].Rows[countRow]["nom"].ToString()+"'))";  
					DataAD.SelectCommand=Comm;
					DataAD.Fill(ds,"n4");
					try
					{
					...
					e.Graphics.DrawString(ds.Tables["fournisseurs"].Rows[countRow]["nom"].ToString(), 
					...
					ds.Tables["n4"].Clear();
					ypos+=15;
					ancienfourn=ds.Tables["fournisseurs"].Rows[countRow]["nom"].ToString();
				}
				countRow+=1;
			}
			...
			if(countRow<(ds.Tables["fournisseurs"].Rows.Count))
				e.HasMorePages=true;
			else
			{
		...
		private void button1_Click(object sender, System.EventArgs e)
		{
			...
			if(radioButton1.Checked)
				Comm.CommandText="SELECT * FROM t_fournisseurs ORDER BY nom";
			else
				Comm.CommandText="SELECT * FROM t_fournisseurs WHERE nom='"+mtgcComboBox1.Text+"' ORDER BY nom";
			DataAD.SelectCommand=Comm;
			DataAD.Fill(ds,"fournisseurs");
			printDocument1.Print();
			countRow=0;
			Page=0;
			ds.Tables["fournisseurs"].Clear();
		}
